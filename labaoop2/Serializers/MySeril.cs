...
namespace labaoop2.Serializers
{
    class MySeril : ISerializer
    {
        ...
        private string recurslisttotext(string result, object obj, int tire)
        {
            ...
            {
                str += "   " + prop.Name + "=";
                if (prop.PropertyType.Name.ToString() != "String" && prop.PropertyType.Name.ToString() != "Int32" && prop.PropertyType.Name.ToString() != "Boolean")
                {
        ...
        public bool Serialize(List<object> listobj, string pathname, Form1 form)
        {
            try
            {
                using (FileStream fs = new FileStream(pathname + ".txt", FileMode.OpenOrCreate))
                {
                    ...
                    MessageBox.Show("Сериализация успешна.", "Cериализация", MessageBoxButtons.OK);
                }
                return true;
            }
            catch { return false; }
        ...
        private void foragregation(object obj, StreamReader fsr)
        {
            ...
            foreach (PropertyInfo prop in props)
            {
                ...
                if (prop.PropertyType.Name.ToString() == "String")
                    ...
                    prop.SetValue(obj, Convert.ToBoolean(correctline));
                else
                {
            ...
            try
            {
                using (StreamReader fsr = new StreamReader(pathname, System.Text.Encoding.Default))
                {
                    ...
                    while ((line = fsr.ReadLine()) != null)
                    {
                        ...
                        else
                        {
                            try
                            {
                                ...
                                else if (props[k].PropertyType.Name.ToString() == "Boolean")
                                    props[k].SetValue(newobj, Convert.ToBoolean(correctline));
                                else
                                {
                   ...
                   form.UpdateListBox(form.listBox1);
                    MessageBox.Show("Десериализация успешна.", "Десериализация", MessageBoxButtons.OK);
                }
                return true;
            }
            catch { return false; }
